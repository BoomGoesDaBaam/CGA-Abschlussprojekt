 ...
 color = emitTexture + diffTexture * vec4(lightColorAmbiente, 1f);
    ...
    for(int i=0;i<5;i++)
    {
        ...
        if(shader == 1)
        {
            ...
            colorSpecular = specTexture * vec4(lightcolor[i], 1f) * spec;
        }
        else
        {
            float cosBeta = max(0.0f, dot(R, V));
            float cosBetak = pow(cosBeta, shininess);// shininess = Glanzvariable
            colorSpecular = specTexture * cosBetak * vec4(lightcolor[i], 1f);//emitPoints = Regenbogen  lightcolor[i]=Normale Lampenfarbe
        }
