...
class Texture2D(imageData: ByteBuffer, width: Int, height: Int, genMipMaps: Boolean): ITexture{
    ...
    override fun setTexParams(wrapS: Int, wrapT: Int, minFilter: Int, magFilter: Int) {
       ...
       //GL_LINEAR == linear interpolation           GL_NEAREST

        //pixel in einer Textur nennt sich Texel
        //Filtering == Soll der nähste Pixel verwendet werden(GL_NEAREST) oder soll linear interpoliert werden?

        //one texel is mapped on multiple pixel
        GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, magFilter)
        //multiple texel are mapped on one pixel
    ...
    override fun bind(textureUnit: Int) {
        //Textur wird aktiviert
        GL13.glActiveTexture(GL13.GL_TEXTURE0 + textureUnit);
        //Textur wird an gerade aktivierte Textureunit gebunden
        //ermöglicht zugriff im shader
        GL11.glBindTexture(GL13.GL_TEXTURE_2D, texID)
    }
