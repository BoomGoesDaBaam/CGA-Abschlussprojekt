...
class ShaderProgram(vertexShaderPath: String, fragmentShaderPath: String) {
    ...
    fun use() {
        ...
        {
            GL20.glUseProgram(programID)            //Benutzung des Shaderprogramms
        }
    }
    ...
    fun setUniform(name: String, value: Vector3f) : Boolean{
        ...
        val loc = GL20.glGetUniformLocation(programID, name)
        if (loc != -1) {
            GL20.glUniform3f(loc, value.get(0), value.get(1), value.get(2))
            return true
        }
        return false
    }

    fun setUniformVec3Array(name: String, value: FloatArray) : Boolean{
        if (programID == 0) return false
        val loc = GL20.glGetUniformLocation(programID, name)
        if (loc != -1) {
    ...
    init {
        ...
        val vSource = String(Files.readAllBytes(vPath))                 //Char-Sequenz
        val fSource = String(Files.readAllBytes(fPath))
        val vShader = GL20.glCreateShader(GL20.GL_VERTEX_SHADER)        //Erstelle Vertex Shader
        if (vShader == 0) throw Exception("Vertex shader object couldn't be created.")
        val fShader = GL20.glCreateShader(GL20.GL_FRAGMENT_SHADER)      //Erstelle Fragment Shader
        if (fShader == 0) {
            GL20.glDeleteShader(vShader)
            throw Exception("Fragment shader object couldn't be created.")
        }
        GL20.glShaderSource(vShader, vSource)                           //Welcher source Code soll verwendet werden
        ...
        GL20.glCompileShader(fShader)                                   //Compile shader for GPU
        ...
        programID = GL20.glCreateProgram()                              //Rendering pass
        if (programID == 0) {
            GL20.glDeleteShader(vShader)
            GL20.glDeleteShader(fShader)
            throw Exception("Program object creation failed.")
        }
        GL20.glAttachShader(programID, vShader)                         //Füge Shader hinzu
        GL20.glAttachShader(programID, fShader)
        GL20.glLinkProgram(programID)                                   //Programm zusammen linken
        ...
        GL20.glDetachShader(programID, vShader)
        GL20.glDetachShader(programID, fShader)
        GL20.glDeleteShader(vShader)                    //Löschung vorgemerkt
        GL20.glDeleteShader(fShader)
    }
}